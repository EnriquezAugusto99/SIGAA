...
$id = $_POST['txt_id'];
$alumno_original = $_POST['txt_alumno_original'];
$materia_original = $_POST['txt_materia_original'];
$docente_original = $_POST['txt_docente_original'];
$curso_original = $_POST['txt_curso_original'];
...
if(empty($id) || empty($alumno) || empty($materia) || empty($docente) || empty($curso) || empty($nota) || empty($tipo_evaluacion) || empty($fecha)){
...
if(!is_numeric($id) || !is_numeric($alumno) || !is_numeric($materia) || !is_numeric($docente) || !is_numeric($curso)){
...
// 5. Validar que la calificación existe
$query_verificar_calificacion = "SELECT id_calificacion FROM calificaciones WHERE id_calificacion = '$id'";
$res_verificar_calificacion = mysqli_query($con, $query_verificar_calificacion);
if(mysqli_num_rows($res_verificar_calificacion) == 0){
    echo '<script>alert("ERROR: La calificación que intenta editar no existe"); window.location="listado_calificacion.php";</script>';
    exit();
}

// 6. Validar que el alumno exista y esté en el curso seleccionado
...
// 7. Validar que la materia exista y pertenezca al curso
...
// 8. Validar que el docente exista, sea docente y esté asignado a esa materia en ese curso
...
// 10. Validar que la fecha corresponda a una instancia (trimestre o recuperatorio)
...
// 11. Actualizar la calificación
$query = "UPDATE calificaciones SET id_alumno = '$alumno', id_materia = '$materia', id_docente = '$docente', trimestre = '$id_trimestre', nota = '$nota', tipo_evaluacion = '" . mysqli_real_escape_string($con, $tipo_evaluacion) . "', fecha = '$fecha' WHERE id_calificacion = '$id'";
$res = mysqli_query($con, $query);

if($res) {
    echo '<script>alert("Calificación actualizada exitosamente"); window.location="listado_calificacion.php";</script>';
    exit();
} else {
    echo '<script>alert("ERROR: No se pudo actualizar la calificación"); history.go(-1);</script>';
}
