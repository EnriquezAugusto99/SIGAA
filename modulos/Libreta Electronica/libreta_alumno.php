...
// Verificamos si hay sesión de alumno
...
// 2. Obtener materias del curso (excluyendo Taller y Tutorías)
$materias = [];
if ($id_curso > 0) {
...
// 3. Obtener notas de materias comunes por trimestre
$notas_acumuladas = [];
...
// 4. Obtener inasistencias por trimestre
...
while ($row = mysqli_fetch_assoc($res_asistencia)) {
    $trim = (int)$row['trimestre'];
    $valor_falta = 1;
    if ($row['tipo_falta'] == 'media') $valor_falta = 0.5;
    if ($row['tipo_falta'] == 'cuarto') $valor_falta = 0.25;
...
// 5. Obtener previas y equivalencias
$previas_pendientes = [];
...
// 6. Obtener datos personales del alumno
...
// 7. Talleres (Solo si NO es ciclo básico)
$talleres_alumno = [];
        ...
        if (!empty($talleres_alumno)) {
            ...
            foreach ($notas_taller_acumuladas as $id_taller => $notas_array) {
                if (count($notas_array) > 0) {
                    $promedio = array_sum($notas_array) / count($notas_array);
                    $trimestre = 1;
                    $notas_taller_trimestre[$trimestre][$id_taller] = truncarNota($promedio, 2);
                }
            }
        }
    }
...
<head>
    ...
    <title>Mi Libreta de Calificaciones</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .volver-btn-libreta { display: inline-block; margin-top: 20px; padding: 10px 24px; background: #710A14; color: white; text-decoration: none; border-radius: 8px; font-weight: 600; }
        .volver-btn-libreta:hover { background: #3F070B; }
        ...
        .lista-previas li { padding: 8px 12px; border-bottom: 1px solid #eee; }
        .badge-pendiente { background: #ff9800; color: white; padding: 2px 8px; border-radius: 12px; font-size: 11px; margin-left: 10px; }
        .no-disponible { color: #999; font-style: italic; }
        
        /* Estilo para celdas no habilitadas */
        .celda-no-habilitada {
            color: #ccc;
            font-style: italic;
            font-size: 11px;
...
<div class="libreta-container">
    ...
    <div class="info-alumno">
        <strong><?php echo htmlspecialchars($alumno_info['Apellido'] ?? '' . ', ' . $alumno_info['Nombre'] ?? ''); ?></strong> | 
        DNI: <?php echo htmlspecialchars($alumno_id); ?> | 
        Curso: <?php echo htmlspecialchars($nombre_curso . '° "' . $division_curso . '"'); ?>
    </div>
            ...
            <table class="tabla-libreta">
                ...
                <tbody>
                    ...
                    <tr style="background: #f0f0f0;">
                        ...
                        <td><strong><?php echo $total_just_anual; ?></strong></td>
                        <td><strong><?php echo $total_injust_anual; ?></strong></td>
                    </tr>
                </tbody>
            </table>
        </div>
                ...
                <?php else: ?>
                    <ul class="lista-previas">
                        <?php foreach($previas_pendientes as $previa): ?>
                            <li><?php echo htmlspecialchars($previa); ?> <span class="badge-pendiente">Pendiente</span></li>
                        <?php endforeach; ?>
                    </ul>
                <?php endif; ?>
            </div>
                ...
                <?php else: ?>
                    <ul class="lista-previas">
                        <?php foreach($equivalencias_pendientes as $equiv): ?>
                            <li><?php echo htmlspecialchars($equiv); ?> <span class="badge-pendiente">Pendiente</span></li>
                        <?php endforeach; ?>
                    </ul>
                <?php endif; ?>
            </div>
    ...
    <div style="text-align: center; margin-top: 35px;">
        <a href="../../recursos/panel.php" class="volver-btn-libreta">← Volver al Panel</a>
    </div>
</div>
